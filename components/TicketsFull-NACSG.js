...
export default {
  template: `
    ...
    <!-- 🟩 SOLID THEME TEMPLATE -->
    <div v-if="theme === 'solid'" v-for="ticket in sortedTickets" :key="ticket.order" :class="getTicketClass(ticket)">
      <div v-if="ticket.label" class="category-label">{{ ticket.label }}</div>
      <div v-if="ticket.seat" class="category-seat-layout">SEAT LAYOUT</div>
    ...
    <!-- 🟦 DEFAULT / DARK THEME TEMPLATE -->
      <div v-else v-for="ticket in sortedTickets" :key="ticket.order" :class="getTicketClass(ticket)">
        <div v-if="ticket.label" class="category-label">{{ ticket.label }}</div>
        <div class="category-header">
          <div v-if="ticket.offer" class="category-offer">{{ ticket.offer }}</div>
          <h3 class="category-title">{{ ticket.category }}</h3>
  ...
  methods: {
    ...
    transformDataFromValues(values) {
      ...
      return values.slice(1).map(row => {
        ...
        if (obj.included) obj.includedArray = obj.included.split('\n').map(item => item.trim().replace(/["]+/g, ''));
        return obj;
      });
    },
